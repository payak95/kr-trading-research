...
def run_combo_scheduler(r, store: AiStore, api_key: str) -> int:
    """1회 배치. due+enabled 콤보 설정만 처리, 나머지는 건너뜀. due 판정은 하위(자식) 타임프레임 기준으로
    ai_shadow_scheduler._due() 를 그대로 재사용(장 상태 게이트 로직 중복 방지 — 하위 프레임에 새 봉이
    생겨야 재판단 의미가 있으므로). 반환=이번 배치에서 새로 기록된 판단 건수."""
    ...
    for name, cfg_json in configs.items():
        ...
        if not _due({"timeframe": child_tf, "interval_min": cfg.get("interval_min")}, last_run):
            continue
        code = cfg.get("symbol", "")
